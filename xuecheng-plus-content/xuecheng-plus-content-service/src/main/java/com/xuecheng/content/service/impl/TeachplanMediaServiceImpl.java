...
public class TeachplanMediaServiceImpl extends ServiceImpl<TeachplanMediaMapper, TeachplanMedia>
    ...
    @Override
    public TeachplanMedia associationMedia(BindTeachplanMediaVO bindTeachplanMediaDto) {
        ...
        if (teachplan == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "教学计划不存在");
        }
        Integer grade = teachplan.getGrade();
        if (grade != 2) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "只允许第二级教学计划绑定媒资文件");
        }
        //课程id
